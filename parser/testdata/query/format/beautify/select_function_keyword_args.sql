...
SELECT
  trim(BOTH ' ' FROM ' x ');
SELECT
  trim(LEADING '0' FROM '00042');
SELECT
  trim(TRAILING '/' FROM '/api/');
SELECT
...
FROM
  t;
SELECT
  overlay('Hello world' PLACING 'SQL' FROM 7);
SELECT
  overlay('Hello world' PLACING 'SQL' FROM 7 FOR 5);
SELECT
  overlayUTF8(s PLACING r FROM x + 1 FOR len(y))
FROM
  t;
SELECT
  max(both),
